...
class __ChoroplethMapState extends State<_ChoroplethMap> {
  ...
  Future<List<MapWardSpendingData>> _loadCSV() async {
    final _rawData =
        await rootBundle.loadString("assets/spending_by_category.csv");
    ...
    for (var i = 1; i < _listData.length; i++) {
      ...
      if (!_years.contains(newData.year)) {
        _years.add(newData.year);
      }
      if (!_categories.contains(newData.category)) {
        _categories.add(newData.category);
      }
  ...
  MapShapeSource createMapShapeSource() {
    if (!_isInitialized) {
      ...
      return MapShapeSource.asset(
        _getBoundaryAssetPath(_selectedYear),
        shapeDataField: 'ward',
        dataCount: _filteredMapData.length,
        primaryValueMapper: (int index) => _filteredMapData[index].ward,
        shapeColorValueMapper: (int index) => _filteredMapData[index].percent,
    ...
    return SfMaps(layers: [
      MapShapeLayer(
        ...
        legend: MapLegend.bar(MapElement.shape,
            title: Text('Percent of Annual Spending')),
          ...
          return Container(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  'Percent Spending: ' +
                      _filteredMapData[index].percent.toStringAsFixed(1) +
                      '%',
                  style: TextStyle(
                      color: Colors.black,
                      fontSize:
    ...
    return Row(
      ...
      children: [
        DropdownButton<int>(
          ...
          items:
              _years.reversed.toList().map<DropdownMenuItem<int>>((int value) {
...
String _getBoundaryAssetPath(int? year) {
  if (year == null) {
    return 'assets/Wards-Boundaries.geojson';
  }
  if (year < 2003) {
    throw Exception('Year must be greater than 2002');
  }
  if (year < 2015) {
    return 'assets/Wards-Boundaries-2003-2015.geojson';
  } else if (year < 2023) {
    return 'assets/Wards-Boundaries-2015-2023.geojson';
  } else {
    return 'assets/Wards-Boundaries.geojson';
  }
}
